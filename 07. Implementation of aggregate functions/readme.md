...
An aggregate function in RDBMS performs a calculation on a set of values and returns a single result. They are:

- SUM: Calculates the total sum of a numeric column in a relational database.
- MIN: Retrieves the minimum value from a column in a relational database table.
- MAX: Retrieves the maximum value from a column in a relational database table.
- COUNT: Counts the number of rows that match a specified condition or the total number of rows in a table.
- AVERAGE: Computes the average (mean) of a numeric column in a relational database table.
...
3. Display the subject, and average mark for each subject.
4. Display the names of subjects for which at least one student got below 40%.
5. Display the name, subject and percentage of marks who got below 40%.
6. Display the faculties and allotted subjects for each faculty.
7. Display the names of faculties who take more than one subject.
8. Display name, subject, mark, and % of mark in ascending order of mark.
